...
net = cv.dnn.readNet(cv.samples.findFile(args.model),
                     cv.samples.findFile(args.config), args.framework)
...
cv.createTrackbar('Confidence threshold, %', winName,
                  int(confThreshold * 100), 99, callback)

cap = cv.VideoCapture(cv.samples.findFileOrKeep(
...
def processingThreadBody():
    ...
    while process:
        ...
        if not frame is None:
            ...
            blob = cv.dnn.blobFromImage(frame, size=(
                inpWidth, inpHeight), swapRB=args.rgb, ddepth=cv.CV_8U)
            ...
            if net.getLayer(0).outputNameToIndex('im_info') != -1:  # Faster-RCNN or R-FCN
                ...
                net.setInput(
                    np.array([[inpHeight, inpWidth, 1.6]], dtype=np.float32), 'im_info')
